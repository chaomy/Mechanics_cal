...
class VAtension(object):
    ...
    def getSij(self):
        ...
        fid.close()
        Sij=np.mat([[s11,s12,s12,0,0,0],[s12,s11,s12,0,0,0],[s12,s12,s11,0,0,0],[0,0,0,s44,0,0],[0,0,0,0,s44,0],[0,0,0,0,0,s44]],"float")
        return Sij
    ...
    def grabStress(self):
        ...
        with open("OUTCAR", 'r') as fid:
            ...
            In = r"\s*"
            for line in fid:
                matchV = re.search(r"\s*volume of cell : \s*" + real_Num,
                                 line)
                matchE = re.search(r"\s*energy\s*without\s*entropy=\s*" + \
...
#	addedstrain[1,2], addedstrain[2,1] = 0.5 * InputStrain[3] ,0.5 * InputStrain[3]
    ...
    def gnposcar(self,delta,Correct_strain):
        ...
        Base_vector=np.mat('2,0,0;0,2,0;0,0,2',"float")
        ...
        with open("poscar",mode='w') as fout:
            ...
            fout.write("30    2\n")
            fout.write("Direct\n")
            fout.write("""0.000000     0.000000     0.000000
...
parser.add_option("-s","--structure",dest="structure",help="fcc bcc or hcp")
parser.add_option("-d","--direction",dest = "orientation", nargs = 6 , type = "int",  help=" x y z yz xz xy", default = "1")
parser.add_option("-e","--element",dest="Element",help="type of Element", default = "Mo")
parser.add_option("-l","--lattice_constant",dest="lattice_constant", type ="float", help="Lattice constant", default=3.15)
...
if __name__ == "__main__" :
	Job = loop_calculate()
	Job.calculate_tensile('W',3.1676,[1,0,0,0,1,0],'bcc')
