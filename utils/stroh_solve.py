...
class Stroh(object):
    ...
    def solve(self, C, burgers, axes=None, tol=1e-8):
        ...
        A = np.array([eigvec[0, :3], eigvec[1, :3], eigvec[2, :3],
                      eigvec[3, :3], eigvec[4, :3], eigvec[5, :3]])
        L = np.array([eigvec[0, 3:], eigvec[1, 3:], eigvec[2, 3:],
                      eigvec[3, 3:], eigvec[4, 3:], eigvec[5, 3:]])
    ...
    def preln(self):
        ...
        ii = np.array([1.j])
        updn = np.array([1, -1, 1, -1, 1, -1])
        factor = ii * np.einsum('s,s,si,sj->ij', updn,
                                self.k, self.L, self.L) / (4 * np.pi)
    ...
    def stress(self, pos):
        ...
        stress = 1 / (2 * np.pi * ii) * \
            np.einsum('a,a,ijkl,al,ak,a,na->nij', updn,
                      self.k, self.__Cijkl, mpn, self.A,
                      self.L.dot(self.__burgers), 1 / eta)
